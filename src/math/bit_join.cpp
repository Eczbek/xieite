...
export namespace xieite {
	...
	[[nodiscard]] constexpr std::bitset<(... + xieite::bit_size<Ts>)> bit_join(Ts... values) noexcept {
		...
		(..., (result = (result >> xieite::bit_size<Ts>) | (std::bitset<(... + xieite::bit_size<Ts>)>(static_cast<xieite::try_unsigned<Ts>>(values)) << ((... + xieite::bit_size<Ts>) - xieite::bit_size<Ts>))));
		return result;
	}
	...
	[[nodiscard]] constexpr std::bitset<xieite::bit_size<T> * size> bit_join(const std::array<T, size>& values) noexcept {
		...
		for (T value : values) {
			result = (result >> xieite::bit_size<T>) | (std::bitset<xieite::bit_size<T> * size>(static_cast<xieite::try_unsigned<T>>(value)) << (xieite::bit_size<T> * (size - 1)));
		}
		return result;
	}
}
