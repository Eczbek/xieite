export module xieite:bit_size;

import std;

export namespace xieite {
	template<typename T>
	constexpr std::size_t size = sizeof(T) * std::numeric_limits<unsigned char>::digits;

	template<std::integral T>
	constexpr std::size_t size<T> = std::numeric_limits<T>::digits + std::numeric_limits<T>::is_signed;
}
