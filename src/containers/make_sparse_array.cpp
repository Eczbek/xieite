...
export namespace xieite::containers {
	...
	noexcept(xieite::concepts::NoThrowInvocable<Functor, Value(std::ranges::range_common_reference_t<Range>)>) {
		static_assert(std::numeric_limits<Key>::digits <= 16, "key type must be reasonably small");
		static_assert(xieite::types::arity<std::ranges::range_value_t<Range>> == 2, "range entries must have one key and one value");
