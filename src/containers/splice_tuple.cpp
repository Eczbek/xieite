...
export namespace xieite::containers {
	...
	[[nodiscard]] constexpr auto spliceTuple(Tuple1&& tuple1, Tuple2&& tuple2 = Tuple2()) noexcept {
		...
		return std::tuple_cat(XIEITE_SEQUENCE(i, start, std::forward_as_tuple(std::get<i>(std::move(tuple1))...)), xieite::containers::forwardTuple(XIEITE_FORWARD(tuple2)), XIEITE_SEQUENCE(i, (tupleSize - end), std::forward_as_tuple(std::get<end + i>(std::move(tuple1))...)));
	}
}
