...
export namespace xieite {
	...
	[[nodiscard]] constexpr std::vector<std::ranges::iterator_t<R>> modes(R& range, F&& comp = {})
	noexcept(xieite::is_nothrow_invoc<F, bool(std::ranges::range_common_reference_t<R>, std::ranges::range_common_reference_t<R>)> && xieite::is_nothrow_range<R>) {
		...
		for (std::size_t i = 1; i < (size - 1); ++i) {
			if (std::invoke(comp, *curr, *prev)) {
				const auto next = std::ranges::next(curr);
				if (std::invoke(comp, *curr, *next)) {
					its.push_back(curr);
				} else if (i == (size - 2)) {
					its.push_back(next);
				}
