...
namespace xieite {
	...
	[[nodiscard]] constexpr auto str_join(R&& range, const xieite::paren<Ch[n]>& delim, std::type_identity_t<std::basic_string_view<Ch, Traits>> pfx, std::type_identity_t<std::basic_string_view<Ch, Traits>> sfx = "", Alloc&& alloc = {})
	...
	[[nodiscard]] constexpr auto str_join(R&& range, const xieite::paren<Ch[n]>& delim, Ch pfx, std::type_identity_t<std::basic_string_view<Ch, Traits>> sfx = "", Alloc&& alloc = {})
		XIEITE_ARROW(xieite::str_join(XIEITE_FWD(range), xieite::str_view<Ch, Traits>(delim), xieite::str_view(pfx), sfx, XIEITE_FWD(alloc)))

	template<std::ranges::input_range R, xieite::is_ch Ch, typename Traits = std::char_traits<Ch>, typename Alloc = std::allocator<Ch>, std::size_t n>
	[[nodiscard]] constexpr auto str_join(R&& range, const xieite::paren<Ch[n]>& delim, std::type_identity_t<std::basic_string_view<Ch, Traits>> pfx, Ch sfx, Alloc&& alloc = {})
		XIEITE_ARROW(xieite::str_join(XIEITE_FWD(range), xieite::str_view<Ch, Traits>(delim), pfx, xieite::str_view(sfx), XIEITE_FWD(alloc)))

	template<std::ranges::input_range R, xieite::is_ch Ch, typename Traits = std::char_traits<Ch>, typename Alloc = std::allocator<Ch>, std::size_t n>
	[[nodiscard]] constexpr auto str_join(R&& range, const xieite::paren<Ch[n]>& delim, Ch pfx, Ch sfx, Alloc&& alloc = {})
