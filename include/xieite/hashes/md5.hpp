#ifndef XIEITE_HEADER_HASHES_MD5
#	define XIEITE_HEADER_HASHES_MD5
...
#	include "../functors/sign_cast.hpp"
...
namespace xieite::hashes {
	...
	[[nodiscard]] constexpr String md5(const xieite::strings::View<String> value) noexcept {
		...
		auto data = std::vector<String::value_type>(value.begin(), value.end());
		const std::uint64_t length = static_cast<std::uint64_t>(data.size()) * 8;
		data.emplace_back(128);
		const auto padding = std::vector<String::value_type>((120 - data.size() % 64) % 64, 0);
		...
		static constexpr xieite::functors::SignCast<std::uint32_t> cast;
		for (std::size_t i = 0; i < dataSize; i += 64) {
			std::array<std::uint64_t, 16> chunks;
			for (std::size_t j = 0; j < 16; ++j) {
				chunks[j] = 0;
				for (std::size_t k = 0; k < 4; ++k) {
					chunks[j] |= cast(data[j + j * 4 + k]) << (k * 8);
				}
			}
