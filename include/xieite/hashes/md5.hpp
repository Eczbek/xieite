...
namespace xieite::hashes {
	...
	[[nodiscard]] constexpr String md5(const xieite::strings::View<String> value) noexcept {
		...
		auto data = std::vector<typename String::value_type>(value.begin(), value.end());
		const std::uint64_t length = static_cast<std::uint64_t>(data.size()) * 8;
		data.emplace_back(128);
		const auto padding = std::vector<typename String::value_type>((120 - data.size() % 64) % 64, '\0');
		const auto lengthBytes = xieite::bits::unjoin<typename String::value_type, 8>(xieite::bits::join(length));
