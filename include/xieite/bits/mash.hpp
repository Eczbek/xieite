...
namespace xieite::bits {
	...
	[[nodiscard]] constexpr std::bitset<(... + sizes)> mash(const Integrals... values) noexcept {
		...
		(..., (result = (result >> sizes) | (std::bitset<(... + sizes)>(static_cast<xieite::types::MaybeUnsigned<Integrals>>(values)) << ((... + sizes) - sizes))));
		return result;
	}
	...
	[[nodiscard]] constexpr std::bitset<(... + sizes)> mash(const std::array<Integral, sizeof...(sizes)>& values) noexcept {
		return ([&values]<std::size_t... i>(std::index_sequence<i...>) {
			std::bitset<(... + sizes)> result;
			(..., (result = (result >> sizes) | (std::bitset<(... + sizes)>(static_cast<xieite::types::MaybeUnsigned<Integral>>(values[i])) << ((... + sizes) - sizes))));
			return result;
