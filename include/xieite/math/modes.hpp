...
namespace xieite::math {
	...
	> [[nodiscard]] constexpr std::vector<xieite::math::Result<Numbers...>> modes(
		const Numbers... values
	) noexcept {
		if constexpr (sizeof...(Numbers)) {
			std::vector<xieite::math::Result<Numbers...>> results;
			for (const auto iterator : xieite::algorithms::modes(
				std::array<xieite::math::Result<Numbers...>, sizeof...(Numbers)> {
