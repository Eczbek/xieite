...
namespace xieite::math {
	...
	[[nodiscard]] constexpr std::vector<std::ranges::range_value_t<Range>> modes(Range&& range) noexcept {
		...
		std::ranges::transform(xieite::algorithms::modes(XIEITE_FORWARD(range), std::ranges::greater()), result.begin(), [](const std::ranges::const_iterator_t<Range> iterator) -> std::ranges::range_const_reference_t<Range> {
			return *iterator;
		});
		return result;
	}
