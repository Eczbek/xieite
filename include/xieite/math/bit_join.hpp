...
namespace xieite::math {
	template<
		std::integral... Integers
	> [[nodiscard]] constexpr std::bitset<(... + xieite::types::sizeBits<Integers>)> bitJoin(
		const Integers... values
	) noexcept {
		std::bitset<(... + xieite::types::sizeBits<Integers>)> result;
		std::size_t shift = (... + xieite::types::sizeBits<Integers>);
		(..., (result |= std::bitset<(... + xieite::types::sizeBits<Integers>)>(
				static_cast<xieite::types::MaybeUnsigned<Integers>>(values)
			)
			<< (shift -= xieite::types::sizeBits<Integers>)
		));
		return result;
	}
}
