...
namespace xieite::math {
	...
	> [[nodiscard]] constexpr std::vector<xieite::math::Interval<Number>> mergeIntervals(
		const IntervalRange& intervals
	) noexcept {
		...
		for (const xieite::math::Interval<Number> interval1 : intervals) {
			...
			for (xieite::math::Interval<Number>& interval2 : result) {
				if (
					min >= interval2.start && min <= interval2.end
					|| max >= interval2.start && max <= interval2.end
				) {
					overlaps = true;
