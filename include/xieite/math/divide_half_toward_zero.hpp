...
namespace xieite::math {
	...
	[[nodiscard]] constexpr Arithmetic_ divideHalfTowardZero(const Arithmetic_ dividend, const Arithmetic_ divisor) noexcept {
		if constexpr (std::floating_point<Arithmetic_>) {
			...
			return static_cast<Arithmetic_>(dividend / divisor + ((dividend % divisor * xieite::math::sign(dividend)) > (divisor / 2 * xieite::math::sign(divisor))) * xieite::math::sign(dividend, divisor));
		}
	}
}
