...
namespace xieite {
	template<xieite::is_char Char, typename Traits, typename VecAlloc = std::allocator<std::basic_string_view<Char, Traits>>>
	...
	template<xieite::is_char Char, typename Traits, typename VecAlloc = std::allocator<std::basic_string_view<Char, Traits>>>
	[[nodiscard]] constexpr std::vector<std::basic_string_view<Char, Traits>, VecAlloc> str_split(std::basic_string_view<Char, Traits> strv, Char delim, bool discard_empty = false, const VecAlloc& vec_alloc = {}) noexcept(false) {
		return xieite::str_split(strv, xieite::make_str_view(delim), discard_empty, XIEITE_FWD(vec_alloc));
	}

	template<xieite::is_char Char, typename Traits, typename VecAlloc = std::allocator<std::basic_string_view<Char, Traits>>, typename Alloc>
	[[nodiscard]] constexpr std::vector<std::basic_string_view<Char, Traits>, VecAlloc> str_split(const std::basic_string<Char, Traits, Alloc>& str, auto&& delim, bool discard_empty = false, const VecAlloc& vec_alloc = {}) noexcept(false) {
		return xieite::str_split(xieite::make_str_view(str), XIEITE_FWD(delim), discard_empty, XIEITE_FWD(vec_alloc));
	}
