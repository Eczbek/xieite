...
namespace xieite {
	template<typename Char, typename Traits = std::char_traits<Char>, typename VecAlloc = std::allocator<std::basic_string_view<Char, Traits>>>
	...
	[[nodiscard]] constexpr auto str_split(const xieite::paren<Char[length]>& str, auto&& delim, bool discard_empty = false, const VecAlloc& vec_alloc = {})
		XIEITE_ARROW(xieite::str_split(xieite::str_view<Char, Traits>(str), XIEITE_FWD(delim), discard_empty, XIEITE_FWD(vec_alloc)))
}
