...
namespace xieite {
	...
	[[nodiscard]] constexpr auto str_replace(std::basic_string_view<Char, Traits> strv, Char orig, std::type_identity_t<std::basic_string_view<Char, Traits>> with, const Alloc& alloc = {})
		XIEITE_ARROW(xieite::str_replace(strv, xieite::make_str_view(orig), with, alloc))
	...
	[[nodiscard]] constexpr auto str_replace(std::basic_string_view<Char, Traits> strv, std::type_identity_t<std::basic_string_view<Char, Traits>> orig, Char with, const Alloc& alloc = {})
		XIEITE_ARROW(xieite::str_replace(strv, orig, xieite::make_str_view(with), alloc))

	template<xieite::is_char Char, typename Traits = std::char_traits<Char>, typename Alloc = std::allocator<Char>>
	[[nodiscard]] constexpr auto str_replace(std::basic_string_view<Char, Traits> strv, Char orig, Char with, const Alloc& alloc = {})
		XIEITE_ARROW(xieite::str_replace(strv, xieite::make_str_view(orig), xieite::make_str_view(with), alloc))

	template<xieite::is_char Char, typename Traits = std::char_traits<Char>, typename Alloc = std::allocator<Char>>
	[[nodiscard]] constexpr auto str_replace(const std::basic_string<Char, Traits, Alloc>& str, auto&& orig, auto&& with, const Alloc& alloc = {})
		XIEITE_ARROW(xieite::str_replace(xieite::make_str_view(str), XIEITE_FWD(orig), XIEITE_FWD(with), alloc))
	...
	[[nodiscard]] constexpr auto str_replace(const xieite::paren<Char[length]>& str, auto&& orig, auto&& with, const Alloc& alloc = {})
		XIEITE_ARROW(xieite::str_replace(xieite::make_str_view<Char, Traits>(str), XIEITE_FWD(orig), XIEITE_FWD(with), alloc))
}
