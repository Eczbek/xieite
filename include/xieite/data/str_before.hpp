...
namespace xieite {
	...
	[[nodiscard]] constexpr auto str_before(const std::basic_string<Char, Traits>& str, auto&& delim)
		XIEITE_ARROW(xieite::str_before(xieite::make_str_view(str), XIEITE_FWD(delim)))
	...
	[[nodiscard]] constexpr auto str_before(const xieite::paren<Char[length]>& str, auto&& delim)
		XIEITE_ARROW(xieite::str_before(xieite::make_str_view<Char, Traits>(str), XIEITE_FWD(delim)))
}
