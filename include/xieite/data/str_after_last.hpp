...
namespace xieite {
	...
	[[nodiscard]] constexpr auto str_after_last(const std::basic_string<Char, Traits>& str, auto&& delim)
		XIEITE_ARROW(xieite::str_after_last(xieite::make_str_view(str), XIEITE_FWD(delim)))
	...
	[[nodiscard]] constexpr auto str_after_last(const xieite::paren<Char[length]>& str, auto&& delim)
		XIEITE_ARROW(xieite::str_after_last(xieite::make_str_view<Char, Traits>(str), XIEITE_FWD(delim)))
}
