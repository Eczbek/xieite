...
namespace xieite {
	template<
		...
		xieite::is_fwd_sized_range Subrange,
	...
	find_occur(
		...
		return xieite::find_occur_if(
			range,
			n,
			std::ranges::size(subrange),
			[&](auto&& other) { return std::ranges::equal(subrange, other, pred); }
		);
	}
	...
	> [[nodiscard]] constexpr std::ranges::iterator_t<Range> find_occur(
		Range& range,
		std::ranges::range_size_t<Range> n,
		std::ranges::range_common_reference_t<const Range> value,
		Pred&& pred = {}
	) noexcept(
		xieite::is_noex_range<Range>
		&& xieite::is_noex_lref_invoc<Pred, bool(
		...
		return xieite::find_occur_if(
			range,
			n,
			[&](auto&& other) { return std::invoke_r<bool>(pred, value, other); }
		);
	}
}
