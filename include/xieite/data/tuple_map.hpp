...
namespace xieite {
	...
	public:
		[[nodiscard]] constexpr tuple_map(std::initializer_list<std::pair<Key, xieite::tuple_map<Container, std::tuple<Keys...>, Value>>> list = {})
			XIEITE_ARROW_CTOR(, map, ((std::from_range, list)))

		[[nodiscard]] constexpr auto operator[](this auto&& self, xieite::is_ref_to<std::tuple<Key, Keys...>> auto&& keys)
			XIEITE_ARROW(std::forward_like<decltype(self)>(self.map.at(std::get<0>(std::move(keys)))[xieite::subtuple<1>(XIEITE_FWD(keys))]))

		constexpr auto insert(xieite::is_ref_to<std::tuple<Key, Keys...>> auto&& keys, xieite::is_ref_to<Value> auto&& value)
	...
	public:
		[[nodiscard]] constexpr tuple_map(std::initializer_list<std::pair<Key, Value>> list = {})
			XIEITE_ARROW_CTOR(, map, ((std::from_range, list)))

		[[nodiscard]] constexpr auto operator[](this auto&& self, xieite::is_ref_to<std::tuple<Key>> auto&& key)
			XIEITE_ARROW(std::forward_like<decltype(self)>(self.map.at(std::get<0>(std::move(key)))))

		constexpr auto insert(xieite::is_ref_to<std::tuple<Key>> auto&& key, xieite::is_ref_to<Value> auto&& value)
