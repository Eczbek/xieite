...
namespace xieite {
	[[nodiscard]] constexpr auto between(
		...
		auto&& after,
		auto&& before,
		auto&& after_pred = std::ranges::equal_to(),
		auto&& before_pred = decltype(after_pred)()
	) XIEITE_ARROW(
		xieite::before_last(
			xieite::after(
				XIEITE_FWD(range),
				XIEITE_FWD(after),
				XIEITE_FWD(after_pred)
			),
			XIEITE_FWD(before),
	...
	[[nodiscard]] constexpr auto between(
		Range&& range,
		auto&& after,
		std::ranges::range_common_reference_t<Range> before,
		auto&& after_pred = std::ranges::equal_to()
	) XIEITE_ARROW(
		xieite::before_last(
			xieite::after(
	...
	[[nodiscard]] constexpr auto between(
		Range&& range,
		std::ranges::range_common_reference_t<Range> after,
		auto&& before,
