...
namespace xieite {
	...
	template<typename Char, typename Traits = std::char_traits<Char>, typename Alloc = std::allocator<Char>>
	[[nodiscard]] constexpr auto pad_front(std::basic_string_view<Char, Traits> strv, std::size_t target_length, Char c = ' ', const Alloc& alloc = {})
		XIEITE_ARROW(xieite::pad_front(std::basic_string<Char, Traits, Alloc>(strv, alloc), target_length, c, alloc))

	template<xieite::is_char Char, typename Traits = std::char_traits<Char>, typename Alloc = std::allocator<Char>, std::size_t length>
	[[nodiscard]] constexpr auto pad_front(const xieite::paren<Char[length]>& str, std::size_t target_length, Char c = ' ', const Alloc& alloc = {})
		XIEITE_ARROW(xieite::pad_front(std::basic_string<Char, Traits, Alloc>(str, length, alloc), target_length, c, alloc))
}
