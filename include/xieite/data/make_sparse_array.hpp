...
namespace xieite {
	template<typename Key, typename Value, std::ranges::input_range Range = std::initializer_list<std::pair<Key, Value>>, xieite::is_invoc<std::pair<Key, Value>(std::ranges::range_common_reference_t<Range>)> Fn = std::identity>
	requires(std::integral<Key> || std::is_enum_v<Key>)
	[[nodiscard]] constexpr std::array<Value, (1uz << xieite::bit_size<Key>)> make_sparse_array(Range&& entries, Fn&& fn = {})
	noexcept(xieite::is_noex_invoc<Fn, std::pair<Key, Value>(std::ranges::range_common_reference_t<Range>)>) {
