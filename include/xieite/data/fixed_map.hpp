...
namespace xieite {
	...
	public:
		...
		: array(xieite::make_array<std::pair<Key, Value>, length>(
			XIEITE_FWD(entries))
		) {}

		[[nodiscard]] explicit(false) constexpr fixed_map(
		...
		[[nodiscard]] constexpr const std::array<std::pair<Key, Value>, length>&
		data() const noexcept {
			return this->array;
		}
	...
	private:
		...
		template<
			typename Map = std::unordered_map<Key, Value*, Hash, Pred, Alloc>
		> [[nodiscard]] Map& get_map() const noexcept {
			static Map map = std::invoke([this] -> Map {
				Map map;
