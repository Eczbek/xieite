...
namespace xieite {
	...
	[[nodiscard]] constexpr std::basic_string_view<Char, Traits> str_after(std::basic_string_view<Char, Traits> strv, auto&& delim) noexcept {
		return xieite::substr(strv, strv.find(delim), std::string_view::npos, xieite::strlen(delim));
	}

	template<xieite::is_char Char, typename Traits, typename Alloc>
	[[nodiscard]] constexpr std::basic_string_view<Char, Traits> str_after(const std::basic_string<Char, Traits, Alloc>& str, auto&& delim) noexcept {
		return xieite::str_after(xieite::make_str_view(str), XIEITE_FWD(delim));
	}
