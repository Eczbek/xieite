...
namespace xieite::geometry {
	struct Point {
		...
		[[nodiscard]] constexpr double angleTo(
			const xieite::geometry::Point point
		) const noexcept {
			return std::fmod(
				std::atan2(this->y - point.y, this->x - point.x) + xieite::math::pi<double>,
				xieite::math::tau<double>
			);
		}

		[[nodiscard]] constexpr double distanceTo(
			const xieite::geometry::Point point
		) const noexcept {
			return std::hypot(this->x - point.x, this->y - point.y);
		}

		[[nodiscard]] constexpr double slopeTo(
			const xieite::geometry::Point point
		) const noexcept {
			return xieite::math::almostEqual(this->x, point.x)
				? std::numeric_limits<double>::infinity()
				: (point.y - this->y) / (point.x - this->x);
		}
	};
}
