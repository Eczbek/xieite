...
namespace xieite::graphics {
	...
	public:
		...
		constexpr Canvas(
			const xieite::geometry::Point center,
			const xieite::geometry::Point radii,
			const xieite::streams::StandardHandle controller = xieite::streams::StandardHandle(std::cin, std::cout)
		) noexcept
			: center(center),
			radii(radii),
			controller(controller),
			data(
				static_cast<std::size_t>(this->radii.x * 2) + 1,
				std::vector<xieite::graphics::Color>(
					static_cast<std::size_t>(this->radii.y * 2) + 1,
					xieite::graphics::Color(255, 255, 255)
				)
			)
		{}

		void print(
			const xieite::streams::Position position
		) const noexcept {
		...
		void draw(
			...
			const xieite::graphics::Color& color
		) noexcept {
			if (
				point.x >= this->center.x - this->radii.x
				&& point.x <= this->center.x + this->radii.x
				&& point.y >= this->center.y - this->radii.y
				&& point.y <= this->center.y + this->radii.y
			) {
				this->data[static_cast<std::size_t>(
					std::round(point.x - this->center.x + this->radii.x)
				)][static_cast<std::size_t>(
					std::round(point.y - this->center.y + this->radii.y)
				)] = color;
			}
		}
			...
			if (line.slope() <= 1 && line.slope() >= -1) {
				for (double x = this->center.x - this->radii.x; x <= this->center.x + this->radii.x; ++x) {
					...
					if (!xieite::math::almostEqual(divisor, 0)) {
						const xieite::geometry::Point intersection = xieite::geometry::Point(
							x,
						...
						if (
							std::same_as<LinearShape, xieite::geometry::Segment>
								? line.start.distanceTo(intersection) + line.end.distanceTo(intersection) - line.length() < 1
								: std::abs((line.end.x - line.start.x)
									* (intersection.y - line.start.y)
									- (line.end.y - line.start.y)
									* (intersection.x - line.start.x)
								) < 1
								&& std::same_as<LinearShape, xieite::geometry::Line>
								|| std::same_as<LinearShape, xieite::geometry::Ray>
								&& (
