...
namespace xieite::functors {
	...
	noexcept(xieite::types::List<Arguments...>::Reverse::template Prepend<Functor>::template Apply<std::is_nothrow_invocable>::value) {
		return std::apply(std::forward<Functor>(functor), xieite::containers::reverseTuple(std::forward_as_tuple(std::forward<Arguments>(arguments)...)));
	}
	...
	noexcept(xieite::types::List<Arguments...>::Reverse::template Prepend<Structure>::template Apply<std::is_nothrow_constructible>::value) {
		return std::make_from_tuple<Structure>(xieite::containers::reverseTuple(std::forward_as_tuple(std::forward<Arguments>(arguments)...)));
	}
}
