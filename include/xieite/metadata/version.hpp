...
namespace xieite::metadata {
	struct Version {
		...
		constexpr Version(
			const std::size_t major = 0,
			const std::size_t minor = 0,
			const std::size_t patch = 0,
		...
		[[nodiscard]] friend constexpr std::strong_ordering operator<=>(
			const xieite::metadata::Version& version1,
			const xieite::metadata::Version& version2
		) noexcept {
		...
		[[nodiscard]] constexpr std::string string() const noexcept {
			std::string result = 'v'
				+ xieite::math::toBase(10, this->major)
				+ '.'
				+ xieite::math::toBase(10, this->minor)
				+ '.' + xieite::math::toBase(10, this->patch);
			if (this->label.size()) {
				result += '-' + this->label;
			}
			return result;
