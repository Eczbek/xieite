...
namespace xieite {
	...
	[[nodiscard]] constexpr auto unroll(auto&& fn, auto&&... args) XIEITE_ARROW(
		([]<auto... i>(xieite::seq<i...>, auto&& fn, auto&&... args) XIEITE_ARROW(
			XIEITE_FWD(fn).template operator()<i...>(XIEITE_FWD(args)...)
		))(xieite::make_seq<count>, XIEITE_FWD(fn), XIEITE_FWD(args)...)
	)

	template<typename... Ts>
	[[nodiscard]] constexpr auto unroll(auto&& fn, auto&&... args) XIEITE_ARROW(
		xieite::unroll<sizeof...(Ts)>(XIEITE_FWD(fn), XIEITE_FWD(args)...)
	)
}
