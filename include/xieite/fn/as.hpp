...
namespace xieite {
	template<typename T>
	constexpr auto as = [][[nodiscard]](std::type_identity_t<T> x) static
		XIEITE_ARROW(x);

	constexpr auto as_c = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::add_c<T&&>>(x));

	constexpr auto as_cv = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::add_cv<T&&>>(x));

	constexpr auto as_v = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::add_v<T&&>>(x));

	constexpr auto as_not_c = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::rm_c<T&&>>(x));

	constexpr auto as_not_cv = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::rm_cv<T&&>>(x));

	constexpr auto as_not_v = []<typename T>[[nodiscard]](T&& x) static
		XIEITE_ARROW(const_cast<xieite::rm_v<T&&>>(x));

	constexpr auto as_sign = []<xieite::is_arith T>(T x) static noexcept {
		return static_cast<xieite::try_sign<T>>(x);
	};

	constexpr auto as_unsign = []<xieite::is_arith T>(T x) static noexcept {
