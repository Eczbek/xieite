...
namespace xieite::strings {
	constexpr std::string hashMd5(const std::string_view value) noexcept {
		...
		std::vector<char> padded = std::vector<char>(value.begin(), value.end());
		const std::uint64_t length = static_cast<std::uint64_t>(padded.size()) * 8;
		padded.emplace_back(128);
		const std::vector<char> padding = std::vector<char>((120 - padded.size() % 64) % 64, 0);
