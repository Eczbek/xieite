...
namespace xieite::strings {
	...
	> [[nodiscard]] constexpr std::string toRomanNumerals(
		const Integer value
	) noexcept {
		...
		return absoluteValue
			? (std::string(absoluteValue / 1000, 'M')
				+ std::string(hundreds[absoluteValue / 100 % 10])
				+ std::string(tens[absoluteValue / 10 % 10])
				+ std::string(units[absoluteValue % 10]))
			: "N";
	}
}
