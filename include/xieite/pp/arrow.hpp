...
#	define DETAIL_XIEITE_ARROW_TRY(noex_, ex_, body_, ...) XIEITE_OPT(body_)(requires XIEITE_OPT(ex_)((XIEITE_UNWRAP(ex_))) { noex_(XIEITE_UNWRAP(body_)); }, true) __VA_OPT__(&& DETAIL_XIEITE_ARROW_TRY_NEXT XIEITE_SCAN(()) (noex_, __VA_ARGS__))
#	define DETAIL_XIEITE_ARROW_TRY_NEXT() DETAIL_XIEITE_ARROW_TRY
#	define DETAIL_XIEITE_ARROW_CATCH(ex_, body_, ...) catch (XIEITE_UNWRAP(XIEITE_OPT(ex_)(ex_, ...))) { return XIEITE_UNWRAP(body_); } __VA_OPT__(DETAIL_XIEITE_ARROW_CATCH_NEXT XIEITE_SCAN(()) (__VA_ARGS__))
