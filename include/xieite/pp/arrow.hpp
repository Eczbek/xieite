...
#define XIEITE_ARROW_DECL(params_, ...) \
	XIEITE_EVAL( \
		...
		-> decltype(auto) \
		requires(DETAIL_XIEITE_ARROW_PARAMS(XIEITE_UNWRAP(params_)) { __VA_ARGS__; }) \
		{ return __VA_ARGS__; } \
	)
#define XIEITE_ARROW_CTOR(body_, ...) \
	XIEITE_EVAL( \
		noexcept(requires { requires(XIEITE_IF(XIEITE_ANY(body_))(noexcept(XIEITE_UNWRAP(body_)) &&) DETAIL_XIEITE_ARROW_NOEX(&&, __VA_ARGS__)); }) \
...
#define DETAIL_XIEITE_ARROW_PARAMS(...) requires __VA_OPT__((DETAIL_XIEITE_EACH_HELPER(DETAIL_XIEITE_ARROW_PARAM, __VA_ARGS__)))
