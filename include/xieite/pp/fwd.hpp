...
#	define DETAIL_XIEITE_FWD_PRVALUE(_capture, ...) \
		([_capture]<bool DETAIL_XIEITE_castable = !::std::is_reference_v<decltype((__VA_ARGS__))> && ::std::same_as<decltype(__VA_ARGS__), decltype(__VA_ARGS__)>, typename DETAIL_XIEITE_type = decltype(__VA_ARGS__)> -> decltype(auto) { \
