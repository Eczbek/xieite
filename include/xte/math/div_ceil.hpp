...
namespace xte {
	[[nodiscard]] constexpr auto div_ceil(xte::is_number auto dividend, xte::is_number auto... divisors) noexcept {
		if constexpr (using common_type = std::common_type_t<decltype(dividend), decltype(divisors)...>; xte::is_float<common_type>) {
			...
			return (..., (quot = xte::ceil(quot / static_cast<common_type>(divisors))));
		} else {
