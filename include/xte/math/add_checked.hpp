...
#	endif
		return (!xte::is_finite(x) || ... || (!xte::is_finite(ys) || ((sum < 0) ? ((std::numeric_limits<Common>::lowest() - sum) > static_cast<Common>(ys)) : ((std::numeric_limits<Common>::max() - sum) < static_cast<Common>(ys))) || (sum += static_cast<Common>(ys), false))) ? xte::null : xte::opt(sum);
	};
}
