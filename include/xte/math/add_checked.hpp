...
#	endif
		return (!xte::is_finite(augend) || ... || (!xte::is_finite(addends) || ((sum < 0) ? ((xte::lowest<common_type> - sum) > xte::cast<common_type>(addends)) : ((xte::highest<common_type> - sum) < xte::cast<common_type>(addends))) || (sum += xte::cast<common_type>(addends), false))) ? xte::null : xte::opt(sum);
	};
}
