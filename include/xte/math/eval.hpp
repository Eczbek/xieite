...
namespace xte {
	...
	constexpr auto eval = [][[nodiscard]](xte::string_view input) static noexcept(false) -> T {
		...
		DETAIL_XTE::eval::data_type result = ([&](this auto expression) -> DETAIL_XTE::eval::data_type {
			...
			auto logic_and = [&] -> DETAIL_XTE::eval::data_type {
				DETAIL_XTE::eval::data_type result = comparison();
				while (true) {
					whitespace();
					if (match("&&")) {
			...
			auto logic_or = [&] -> DETAIL_XTE::eval::data_type {
				DETAIL_XTE::eval::data_type result = logic_and();
				while (true) {
					whitespace();
					if (match("||")) {
