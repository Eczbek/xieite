...
#	endif
		bool overflow;
		(void)(!xte::is_finite(prod) || ... || (xte::approx_equal(prod, 0) || (overflow = !xte::is_finite(ys) || (xte::abs(static_cast<Common>(ys)) > (xte::abs(((prod < 0) == (ys < 0)) ? std::numeric_limits<Common>::max() : std::numeric_limits<Common>::lowest()) / xte::abs(prod)))) || !xte::assign(prod, prod * static_cast<Common>(ys))));
