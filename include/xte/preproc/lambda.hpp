...
#
#	define XTE_LAMBDA(...) ([]DETAIL_XTE_LAMBDA(0, 0, __VA_ARGS__))
#	define XTE_LAMBDA_THIS(...) ([]DETAIL_XTE_LAMBDA(0, 1, __VA_ARGS__))
#	define XTE_LAMBDA_LOCAL(...) ([__VA_ARGS__]DETAIL_XTE_LAMBDA_LOCAL
#	define XTE_LAMBDA_MUTABLE(...) (DETAIL_XTE::LAMBDA::indirect { [__VA_ARGS__] mutable { return [&]DETAIL_XTE_LAMBDA(1, 1, __VA_ARGS__); } })

XTE_DIAGNOSTIC_OFF_CLANG("-Wdollar-in-identifier-extension")

#	define DETAIL_XTE_LAMBDA_LOCAL(...) DETAIL_XTE_LAMBDA_DECL(1, 1, __VA_ARGS__))
...
namespace DETAIL_XTE::LAMBDA {
	template<typename Base>
	struct indirect : Base {
		template<decltype(auto)... args0>
		[[nodiscard]] constexpr auto operator()(this Base& self, auto&&... args1) XTE_ARROW(
			self.template operator()<args0...>(XTE_FWD(args1)...)
		)
	};
}
