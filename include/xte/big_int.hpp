#ifndef DETAIL_XTE_HEADER_BIG_INT
#	define DETAIL_XTE_HEADER_BIG_INT
#
#	include "./abs.hpp"
#	include "./address.hpp"
#	include "./aliases.hpp"
...
namespace xte {
	...
	private:
		...
		[[nodiscard]] constexpr xte::big_int& _mul(auto&& rhs) {
			...
			do {
				...
				if ((this->_data.size() < 2) && (copy._data.size() < 2)) {
					auto [lo, hi] = xte::wide_int<xte::umax>(this->_data[0]) * copy._data[0];
					this->_data[0] = lo;
					if (hi) {
		...
		[[nodiscard]] constexpr xte::big_int _pow(this auto&& base, auto&& exp) {
			...
			if (exp < 0) {
				if (!base) {
					throw xte::static_error<"power of zero to negative exponent">();
				}
				return 0;
			}
			if (!exp) {
	...
	public:
		...
		constexpr xte::big_int& operator/=(const xte::big_int& rhs) & noexcept(false) {
			...
			if ((this->_data.size() < 3) && (rhs._data.size() < 3)) {
				auto [lo, hi] = xte::wide_int<xte::umax>(this->_data[0], (this->_data.size() > 1) ? this->_data[1] : 0) / xte::wide_int<xte::umax>(rhs._data[0], (rhs._data.size() > 1) ? rhs._data[1] : 0);
		...
		[[nodiscard]] constexpr xte::big_int root(const xte::big_int& degree) const noexcept(false) {
			...
			if (!degree) {
				throw xte::static_error<"root of zeroth degree">();
			}
			if (*this == 1) {
				return *this;
			}
		...
		[[nodiscard]] constexpr xte::big_int log(const xte::big_int& base) const noexcept(false) {
			...
			if (this->_neg) {
				throw xte::static_error<"logarithm of negative anti-logarithm">();
			}
			if (base == 1) {
				throw xte::static_error<"logarithm to unary base">();
			}
			if (base._neg) {
				throw xte::static_error<"logarithm to negative base">();
			}
		...
		[[nodiscard]] constexpr xte::string str(this auto&& self, radix_type radix = 10, const xte::serialize_config& config = {}) noexcept(false) {
			...
			do {
				...
				if constexpr (xte::is_signed_int<decltype(radix)>) {
					if (radix == -1) {
