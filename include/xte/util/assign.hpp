...
namespace xte {
	...
	constexpr auto assign(T&& lhs, U&& rhs) XTE_RETURNS_FIRST(
		([](T& lhs, U rhs) noexcept -> T& requires(are_arithmetic) {
			if constexpr (are_arithmetic) {
				return lhs = xte::make<xte::drop_cvref<T>>(rhs);
			}
			std::unreachable();
		})(lhs, rhs),
		XTE_FWD(lhs) = XTE_FWD(rhs),
		XTE_FWD(lhs) = xte::make<xte::drop_cvref<decltype(lhs)>>(XTE_FWD(rhs)),
		xte::reconstruct(lhs, XTE_FWD(rhs)),
		xte::reconstruct(lhs, xte::make<xte::drop_cvref<decltype(lhs)>>(XTE_FWD(rhs)))
	)
