#ifndef DETAIL_XTE_HEADER_TRAIT_IS_RANGE_NOEX
#	define DETAIL_XTE_HEADER_TRAIT_IS_RANGE_NOEX
#
...
namespace xte {
	...
	concept is_range_noex =
		std::ranges::range<T>
		&& xte::is_iter_noex<std::ranges::iterator_t<T>>
		&& xte::is_sentinel_noex<std::ranges::sentinel_t<T>, std::ranges::iterator_t<T>>
		&& requires(T range) {
