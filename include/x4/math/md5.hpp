...
#include <string_view>
#include <vector>
#include "../math/bitj.hpp"
#include "../math/sgncast.hpp"
#include "../math/strn.hpp"
#include "../math/bitsplit.hpp"
...
namespace x4 {
	[[nodiscard]] constexpr std::string md5(std::string_view str) noexcept {
		...
		const auto size_bytes = x4::bitsplit<char, 8>(x4::bitj(static_cast<std::uint64_t>(data.size()) * 8));
		...
		for (std::size_t i = 0; i < data.size(); i += 64) {
			...
			for (std::size_t j = 0; j < 16; ++j) {
				...
				for (std::size_t k = 0; k < 4; ++k) {
					chunks[j] |= x4::sgncast<std::uint32_t>(data[j + j * 4 + k]) << (k * 8);
				}
			}
		...
		for (std::uint32_t item : foo) {
			result += x4::strn((item >> 24) | ((item & 16711680) >> 8) | ((item & 65280) << 8) | (item << 24), 16, {}, 8);
		}
		return result;
	}
}
