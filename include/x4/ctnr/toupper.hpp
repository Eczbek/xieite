...
namespace x4 {
	...
	[[nodiscard]] constexpr Ch toupper(Ch c) noexcept {
		...
		static constexpr Lookup lookup = ([] static -> Lookup {
			...
			for (std::size_t i = 0; i < x4::ch::alphabetsz; ++i) {
				lookup[x4::sgncast<std::size_t>(x4::ch::lower[i])] = static_cast<Ch>(x4::ch::upper[i]);
			}
			return lookup;
		})();
	...
	[[nodiscard]] constexpr auto toupper(const std::basic_string<Ch, Traits, Alloc>& str, Alloc&& alloc = {})
		X4AR(x4::toupper(x4::strv(str), X4FWD(alloc)))

	template<x4::isch Ch, typename Traits = std::char_traits<Ch>, typename Alloc = std::allocator<Ch>, std::size_t n>
	[[nodiscard]] constexpr auto toupper(const Ch(& str)[n], Alloc&& alloc = {})
		X4AR(x4::toupper(x4::strv<Ch, Traits>(str), X4FWD(alloc)))
}
