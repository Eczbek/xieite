...
[[nodiscard]] constexpr std::ranges::subrange<std::ranges::iterator_t<Range>> findMaximumConsecutive(Range&& range, std::ranges::range_const_reference_t<Range> value, Comparator&& comparator = Comparator()) noexcept;
```
...
#include <print>
#include <vector>
#include "xieite/algorithms/find_maximum_consecutive.hpp"
...
int main() {
    ...
    auto result = xieite::algorithms::findMaximumConsecutive(values, 1);
